...
public class OracleTextToWagic {
    static void parseOracleText(JSONArray keywords, String oracleText,
        String cardName, String type, String subtype, String power,
        String manaCost, FileWriter myWriter) throws IOException {
        String abilities = "";
        // Remove remainder text        
        oracleText = oracleText.replaceAll("\\(.*?\\)", "").replace("()", "");
        // Remove comma from Legendaries
        oracleText = oracleText.replaceAll(cardName.replace(",", ""),
                cardName.replace(",", ""));
        ...
        if ((keywords == null) &&
                (type.contains("Creature") || subtype.contains("Vehicle"))) {
        ...
        for (String oracleBit : oracleText.split("\n")) {
            ...
            if (!(type.contains("Instant") || type.contains("Sorcery") ||
                ...
                autoLineExists(myWriter,
                    AutoEffects.DetermineActivatedAbility(oracleBit, cardName,
                        type, subtype));
                autoLineExists(myWriter,
                    AutoLine.ManaAbility(oracleBit, subtype));
				...
				autoLineExists(myWriter, AutoLine.Backup(oracleBit));

                autoLineExists(myWriter, AutoLineGRN.Proliferate(oracleBit));
				autoLineExists(myWriter, AutoLineGRN.Convoke(oracleBit));
            ...
            if (type.contains("Instant") || type.contains("Sorcery")) {
                ...
                autoLineExists(myWriter,
                    AutoLine.MyTarget(oracleBit, "InstantOrSorcery", subtype));
                autoLineExists(myWriter,
                    AutoEffects.ProcessEffect(oracleBit, type));
                autoLineExists(myWriter,
                    AutoLine.ExileDestroyDamage(oracleBit, type));
                autoLineExists(myWriter,
                    AutoLine.ReplacerAuraEquipBonus(oracleBit,
                        "InstantOrSorcery"));
            ...
            if (subtype.contains("Aura")) {
                autoLineExists(myWriter, AutoLine.Corrupted(oracleBit, cardName));
                autoLineExists(myWriter,
                    AutoLine.MyTarget(oracleBit, subtype, subtype));
                autoLineExists(myWriter, AutoLineGRN.Convoke(oracleBit));

                //autoLineExists(AutoLine.Triggers(oracleBit, cardName, type, subtype, power));
                String auraEquipBonus = AutoLine.ReplacerAuraEquipBonus(oracleBit,
                        subtype);
            ...
            if (subtype.contains("Equipment")) {
                //autoLineExists(AutoLine.Triggers(oracleBit, cardName, type, subtype, power));
                autoLineExists(myWriter,
                    AutoLine.AuraEquipBonus(oracleBit, "Equipment"));
                autoLineExists(myWriter,
                    AutoLine.ForMirrodin(oracleBit, "Equipment"));
            ...
            if (subtype.contains("Saga")) {
                autoLineExists(myWriter, AutoLine.EpicSaga(oracleBit, subtype));
            }
        }
    }

    static void autoLineExists(FileWriter myWriter, String outcome,
        boolean... needAuto) throws IOException {
        if (outcome.length() > 0) {
